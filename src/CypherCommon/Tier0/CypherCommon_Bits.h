...
CYPHER_NODISCARD constexpr u32 Cy_LowMask32( u32 nBitCount ) noexcept
{
    return nBitCount >= 32u ? CY_U32_MAX : ( ( 1u << nBitCount ) - 1u );
}

// Builds a mask with the lowest bit_count 64-bit bits set.
CYPHER_NODISCARD constexpr u64 Cy_LowMask64( u32 nBitCount ) noexcept
{
    return nBitCount >= 64u ? CY_U64_MAX : ( ( 1ull << nBitCount ) - 1ull );
}

// Returns true when any requested flag is set.
CYPHER_NODISCARD constexpr bool_t Cy_HasAnyFlags( u32 nValue, u32 nFlags ) noexcept
{
    return ( nValue & nFlags ) != 0u;
}

// Returns true when all requested flags are set.
CYPHER_NODISCARD constexpr bool_t Cy_HasAllFlags( u32 nValue, u32 nFlags ) noexcept
{
    return ( nValue & nFlags ) == nFlags;
}

// Returns value with flags enabled.
CYPHER_NODISCARD constexpr u32 Cy_SetFlags( u32 nValue, u32 nFlags ) noexcept
{
    return nValue | nFlags;
}

// Returns value with flags disabled.
CYPHER_NODISCARD constexpr u32 Cy_ClearFlags( u32 nValue, u32 nFlags ) noexcept
{
    return nValue & ~nFlags;
}

// Returns value with flags flipped.
CYPHER_NODISCARD constexpr u32 Cy_ToggleFlags( u32 nValue, u32 nFlags ) noexcept
{
    return nValue ^ nFlags;
}

// 64-bit variants for large feature and category masks.
CYPHER_NODISCARD constexpr bool_t Cy_HasAnyFlags( u64 nValue, u64 nFlags ) noexcept
{
    return ( nValue & nFlags ) != 0ull;
}

CYPHER_NODISCARD constexpr bool_t Cy_HasAllFlags( u64 nValue, u64 nFlags ) noexcept
{
    return ( nValue & nFlags ) == nFlags;
}

CYPHER_NODISCARD constexpr u64 Cy_SetFlags( u64 nValue, u64 nFlags ) noexcept
{
    return nValue | nFlags;
}

CYPHER_NODISCARD constexpr u64 Cy_ClearFlags( u64 nValue, u64 nFlags ) noexcept
{
    return nValue & ~nFlags;
}

CYPHER_NODISCARD constexpr u64 Cy_ToggleFlags( u64 nValue, u64 nFlags ) noexcept
{
    return nValue ^ nFlags;
}

// Returns true when a specific bit is set in a 32-bit value.
CYPHER_NODISCARD constexpr bool_t Cy_TestBit32( u32 nValue, u32 nBit ) noexcept
{
    return ( nValue & Cy_Bit32( nBit ) ) != 0u;
}

// Returns true when a specific bit is set in a 64-bit value.
CYPHER_NODISCARD constexpr bool_t Cy_TestBit64( u64 nValue, u32 nBit ) noexcept
{
    return ( nValue & Cy_Bit64( nBit ) ) != 0u;
}

// Returns value with a specific 32-bit bit enabled.
CYPHER_NODISCARD constexpr u32 Cy_SetBit32( u32 nValue, u32 nBit ) noexcept
{
    return nValue | Cy_Bit32( nBit );
}

// Returns value with a specific 64-bit bit enabled.
CYPHER_NODISCARD constexpr u64 Cy_SetBit64( u64 nValue, u32 nBit ) noexcept
{
    return nValue | Cy_Bit64( nBit );
}

// Returns value with a specific 32-bit bit disabled.
CYPHER_NODISCARD constexpr u32 Cy_ClearBit32( u32 nValue, u32 nBit ) noexcept
{
    return nValue & ~Cy_Bit32( nBit );
}

// Returns value with a specific 64-bit bit disabled.
CYPHER_NODISCARD constexpr u64 Cy_ClearBit64( u64 nValue, u32 nBit ) noexcept
{
    return nValue & ~Cy_Bit64( nBit );
}

// Counts set bits in a 32-bit value.
CYPHER_NODISCARD constexpr u32 Cy_PopCount32( u32 nValue ) noexcept
{
    return static_cast<u32>( std::popcount( nValue ) );
}

// Counts set bits in a 64-bit value.
CYPHER_NODISCARD constexpr u32 Cy_PopCount64( u64 nValue ) noexcept
{
    return static_cast<u32>( std::popcount( nValue ) );
}

// Counts zero bits before the highest set 32-bit bit.
CYPHER_NODISCARD constexpr u32 Cy_CountLeadingZeros32( u32 nValue ) noexcept
{
    return static_cast<u32>( std::countl_zero( nValue ) );
}

// Counts zero bits before the highest set 64-bit bit.
CYPHER_NODISCARD constexpr u32 Cy_CountLeadingZeros64( u64 nValue ) noexcept
{
    return static_cast<u32>( std::countl_zero( nValue ) );
}

// Counts zero bits after the lowest set 32-bit bit.
CYPHER_NODISCARD constexpr u32 Cy_CountTrailingZeros32( u32 nValue ) noexcept
{
    return static_cast<u32>( std::countr_zero( nValue ) );
}

// Counts zero bits after the lowest set 64-bit bit.
CYPHER_NODISCARD constexpr u32 Cy_CountTrailingZeros64( u64 nValue ) noexcept
{
    return static_cast<u32>( std::countr_zero( nValue ) );
}

// Returns the index of the lowest set bit, or -1 when value is zero.
CYPHER_NODISCARD constexpr i32 Cy_FindLowestSetBit32( u32 nValue ) noexcept
{
    return nValue == 0u ? -1 : static_cast<i32>( Cy_CountTrailingZeros32( nValue ) );
}

// Returns the index of the lowest set bit, or -1 when value is zero.
CYPHER_NODISCARD constexpr i32 Cy_FindLowestSetBit64( u64 nValue ) noexcept
{
...
CYPHER_NODISCARD constexpr i32 Cy_FindHighestSetBit32( u32 nValue ) noexcept
{
    return nValue == 0u ? -1 : static_cast<i32>( 31u - Cy_CountLeadingZeros32( nValue ) );
}

// Returns the index of the highest set bit, or -1 when value is zero.
CYPHER_NODISCARD constexpr i32 Cy_FindHighestSetBit64( u64 nValue ) noexcept
{
...
// Rounds up to the next power of two. Zero returns one and overflow returns zero.
CYPHER_NODISCARD constexpr u64 Cy_NextPowerOfTwo64( u64 nValue ) noexcept
{
    if ( nValue <= 1ull ) {
        return 1ull;
    }
...
CYPHER_NODISCARD constexpr bool_t Cy_NextPowerOfTwo32Checked( u32 nValue, u32 &nOutValue ) noexcept
{
    nOutValue = Cy_NextPowerOfTwo32( nValue );
    return nOutValue != 0u;
}

CYPHER_NODISCARD constexpr bool_t Cy_NextPowerOfTwo64Checked( u64 nValue, u64 &nOutValue ) noexcept
{
...
CYPHER_NODISCARD constexpr u32 Cy_PreviousPowerOfTwo32( u32 nValue ) noexcept
{
    return nValue == 0u ? 0u : Cy_Bit32( static_cast<u32>( Cy_FindHighestSetBit32( nValue ) ) );
}

// Rounds down to the previous power of two; zero returns zero.
CYPHER_NODISCARD constexpr u64 Cy_PreviousPowerOfTwo64( u64 nValue ) noexcept
{
    return nValue == 0ull ? 0ull : Cy_Bit64( static_cast<u32>( Cy_FindHighestSetBit64( nValue ) ) );
}

// Rotates a 32-bit value left by shift bits.
CYPHER_NODISCARD constexpr u32 Cy_RotateLeft32( u32 nValue, i32 nShift ) noexcept
{
    return std::rotl( nValue, nShift );
}

// Rotates a 32-bit value right by shift bits.
CYPHER_NODISCARD constexpr u32 Cy_RotateRight32( u32 nValue, i32 nShift ) noexcept
{
    return std::rotr( nValue, nShift );
}

// Rotates a 64-bit value left by shift bits.
CYPHER_NODISCARD constexpr u64 Cy_RotateLeft64( u64 nValue, i32 nShift ) noexcept
{
    return std::rotl( nValue, nShift );
}

// Rotates a 64-bit value right by shift bits.
CYPHER_NODISCARD constexpr u64 Cy_RotateRight64( u64 nValue, i32 nShift ) noexcept
{
    return std::rotr( nValue, nShift );
}
...
CYPHER_NODISCARD constexpr u32 Cy_BitRangeMask32( u32 nBitOffset, u32 nBitCount ) noexcept
{
...
CYPHER_NODISCARD constexpr u64 Cy_BitRangeMask64( u32 nBitOffset, u32 nBitCount ) noexcept
{
...
CYPHER_NODISCARD constexpr u32 Cy_ExtractBits32( u32 nValue, u32 nBitOffset, u32 nBitCount ) noexcept
{
    return nBitOffset < 32u
...
CYPHER_NODISCARD constexpr u64 Cy_ExtractBits64( u64 nValue, u32 nBitOffset, u32 nBitCount ) noexcept
{
    return nBitOffset < 64u
...
CYPHER_NODISCARD constexpr u32 Cy_ReplaceBits32(
    u32 nValue,
    u32 nFieldValue,
    u32 nBitOffset,
...
CYPHER_NODISCARD constexpr u64 Cy_ReplaceBits64(
    u64 nValue,
    u64 nFieldValue,
    u32 nBitOffset,
    u32 nBitCount ) noexcept
