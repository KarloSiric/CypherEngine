...
//
//  File: tests/CypherFileSystem/CypherFileSystem_Smoke.cpp
//  Purpose: Tests FileSystem Smoke behavior.
...
bool CheckError( fs::fs_error_t actual, fs::fs_error_t expected, const char *message )
{
    ...
    std::fprintf(
        ...
        message,
        fs::FS_ErrorName( expected ),
        fs::FS_ErrorName( actual ) );
    return false;
}
...
int main()
{
    ...
    if ( !CheckError( fs::FS_Init(), fs::fs_error_t::OK, "init" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_SetWritePath( szWritePath.c_str() ), fs::fs_error_t::OK, "set write path" ) ) {
        return 1;
    }

    char szPathBuffer[fs::CYPHER_FILESYSTEM_MAX_PATH_LENGTH]{};
    if ( !CheckError( fs::FS_NormalizeVirtualPath( "Profiles\\Player1\\CONFIG.CFG", szPathBuffer, sizeof( szPathBuffer ) ), fs::fs_error_t::OK, "normalize virtual path" ) ) {
        return 1;
    }
    if ( !Check( std::strcmp( szPathBuffer, "profiles/player1/config.cfg" ) == 0, "normalized path value" ) ) {
        return 1;
    }
    if ( !Check( fs::FS_IsValidVirtualPath( "textures/walls/stone.dds" ), "valid virtual path" ) ) {
        return 1;
    }
    ...
    for ( const invalid_virtual_path_case_t &invalidPath : invalidVirtualPaths ) {
        if ( !Check( !fs::FS_IsValidVirtualPath( invalidPath.path ), invalidPath.message ) ) {
            return 1;
        }
        if ( !CheckError( fs::FS_NormalizeVirtualPath( invalidPath.path, szPathBuffer, sizeof( szPathBuffer ) ), fs::fs_error_t::ERR_INVALID_PATH, invalidPath.message ) ) {
            return 1;
        }
    }
    if ( !CheckError( fs::FS_PathJoin( "profiles", "Player1\\CONFIG.CFG", szPathBuffer, sizeof( szPathBuffer ) ), fs::fs_error_t::OK, "path join" ) ) {
        return 1;
    }
    if ( !Check( std::strcmp( szPathBuffer, "profiles/player1/config.cfg" ) == 0, "path join value" ) ) {
        return 1;
    }
    if ( !Check( std::strcmp( fs::FS_PathBasename( "profiles/player1/config.cfg" ), "config.cfg" ) == 0, "path basename" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_PathDirname( "Profiles\\Player1\\CONFIG.CFG", szPathBuffer, sizeof( szPathBuffer ) ), fs::fs_error_t::OK, "path dirname" ) ) {
        return 1;
    }
    if ( !Check( std::strcmp( szPathBuffer, "profiles/player1" ) == 0, "path dirname value" ) ) {
        return 1;
    }
    if ( !Check( std::strcmp( fs::FS_PathExtension( "profiles/player1/config.cfg" ), ".cfg" ) == 0, "path extension" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_PathWithoutExtension( "Profiles\\Player1\\CONFIG.CFG", szPathBuffer, sizeof( szPathBuffer ) ), fs::fs_error_t::OK, "path without extension" ) ) {
        return 1;
    }
    if ( !Check( std::strcmp( szPathBuffer, "profiles/player1/config" ) == 0, "path without extension value" ) ) {
        return 1;
    }
    if ( !Check( fs::FS_PathHasExtension( "profiles/player1/config.cfg", "CFG" ), "path has extension" ) ) {
        return 1;
    }

    TraceStep( "basic write and read mount" );

    if ( !CheckError( fs::FS_CreateDirectory( "profiles/player1" ), fs::fs_error_t::OK, "create directory" ) ) {
        return 1;
    }

    fs::file_t file{};
    if ( !CheckError( fs::FS_Open( "profiles/player1/config.cfg", fs::open_mode_t::WRITE_TEXT, file ), fs::fs_error_t::OK, "open write" ) ) {
        return 1;
    }

    const char text[] = "name player1\n";
    cypher::engine::common::u64 nBytesWritten = 0u;
    if ( !CheckError( fs::FS_Write( file, text, std::strlen( text ), nBytesWritten ), fs::fs_error_t::OK, "write file" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_MountDirectoryWithHandle( "writable", szWritePath.c_str(), fs::CYPHER_FILESYSTEM_MOUNT_WRITABLE, 0u, hWritableMount ), fs::fs_error_t::ERR_NOT_IMPLEMENTED, "writable directory mount not implemented" ) ) {
        return 1;
    }
    if ( !Check( hWritableMount == fs::CYPHER_FILESYSTEM_INVALID_MOUNT, "writable directory mount leaves handle invalid" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_MountDirectory( "mixed", szWritePath.c_str(), fs::CYPHER_FILESYSTEM_MOUNT_READ_ONLY | fs::CYPHER_FILESYSTEM_MOUNT_WRITABLE, 0u ), fs::fs_error_t::ERR_NOT_IMPLEMENTED, "mixed writable directory mount not implemented" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_MountDirectoryWithHandle( "", szWritePath.c_str(), fs::CYPHER_FILESYSTEM_MOUNT_READ_ONLY, 0u, readMount ), fs::fs_error_t::OK, "mount write path for read view" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_GetMountInfoByHandle( readMount, mountInfo ), fs::fs_error_t::OK, "get mount info by handle" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_MountDirectoryWithHandle( "game", szBaseContentPath.string().c_str(), fs::CYPHER_FILESYSTEM_MOUNT_READ_ONLY, 1u, pBaseMount ), fs::fs_error_t::OK, "mount base overlay" ) ) {
        return 1;
    }
    fs::mount_handle_t modMount = fs::CYPHER_FILESYSTEM_INVALID_MOUNT;
    if ( !CheckError( fs::FS_MountDirectoryWithHandle( "game", szModContentPath.string().c_str(), fs::CYPHER_FILESYSTEM_MOUNT_READ_ONLY, 2u, modMount ), fs::fs_error_t::OK, "mount mod overlay" ) ) {
        return 1;
    }

    if ( !CheckError( fs::FS_ResolvePath( "game/shared.cfg", szPathBuffer, sizeof( szPathBuffer ) ), fs::fs_error_t::OK, "resolve overlay priority" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_ListDirectory( "game", pOverlayEntries, 8u, nOverlayEntryCount ), fs::fs_error_t::OK, "list overlay directory" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_Unmount( modMount ), fs::fs_error_t::OK, "unmount overlay by handle" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_GetMountInfoByHandle( modMount, mountInfo ), fs::fs_error_t::ERR_MOUNT_NOT_FOUND, "unmounted handle not found" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_ResolvePath( "game/shared.cfg", szPathBuffer, sizeof( szPathBuffer ) ), fs::fs_error_t::OK, "resolve overlay after unmount" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_MountPackage( "game", szPackageArchivePathString.c_str(), fs::CYPHER_FILESYSTEM_MOUNT_READ_ONLY, 3u ), fs::fs_error_t::OK, "mount package overlay" ) ) {
        return 1;
    }
    if ( !Check( fs::FS_PackageIsMounted( szPackageArchivePathString.c_str() ), "package is mounted" ) ) {
        return 1;
    }
    fs::package_info_t packageInfo{};
    if ( !CheckError( fs::FS_GetPackageInfo( szPackageArchivePathString.c_str(), packageInfo ), fs::fs_error_t::OK, "get package info" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_ResolvePath( "game/shared.cfg", szPathBuffer, sizeof( szPathBuffer ) ), fs::fs_error_t::ERR_UNSUPPORTED_BACKEND, "resolve package overlay unsupported" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_ReadEntireFile( "game/shared.cfg", pPackageReadBuffer, sizeof( pPackageReadBuffer ), nPackageBytesRead ), fs::fs_error_t::OK, "read package overlay file" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_Open( "game/shared.cfg", fs::open_mode_t::READ_BINARY, packageFile ), fs::fs_error_t::OK, "open package file" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_Tell( packageFile, nPackagePosition ), fs::fs_error_t::OK, "tell package file start" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_Read( packageFile, pPackageHandleBuffer, 4u, nPackageBytesRead ), fs::fs_error_t::OK, "read package file prefix" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_Seek( packageFile, 5, fs::seek_origin_t::CYPHER_FILESYSTEM_SEEK_START ), fs::fs_error_t::OK, "seek package file near end" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_Read( packageFile, pPackageHandleBuffer, sizeof( pPackageHandleBuffer ), nPackageBytesRead ), fs::fs_error_t::OK, "read package file short at eof" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_Tell( packageFile, nPackagePosition ), fs::fs_error_t::OK, "tell package file eof" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_Read( packageFile, pPackageHandleBuffer, sizeof( pPackageHandleBuffer ), nPackageBytesRead ), fs::fs_error_t::OK, "read package file at eof" ) ) {
        return 1;
    }
    if ( !Check( nPackageBytesRead == 0u && pPackageHandleBuffer[0] == 'x', "package file eof read returns zero bytes" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_Read( packageFile, nullptr, 0u, nPackageBytesRead ), fs::fs_error_t::OK, "zero read package file" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_Write( packageFile, "x", 1u, nPackageBytesWritten ), fs::fs_error_t::ERR_PERMISSION_DENIED, "write package file denied" ) ) {
        return 1;
    }
    if ( !Check( nPackageBytesWritten == 0u, "package file denied write count" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_Flush( packageFile ), fs::fs_error_t::ERR_PERMISSION_DENIED, "flush package file denied" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_Close( packageFile ), fs::fs_error_t::OK, "close package file" ) ) {
        return 1;
    }

    fs::file_info_t packageFileInfo{};
    if ( !CheckError( fs::FS_GetFileInfo( "game/shared.cfg", packageFileInfo ), fs::fs_error_t::OK, "package file info" ) ) {
        return 1;
    }
    ...
    if ( !Check( fs::FS_DirectoryExists( "game/scripts" ), "package virtual directory exists" ) ) {
        return 1;
    }

    if ( !CheckError( fs::FS_ListDirectory( "game", pOverlayEntries, 8u, nOverlayEntryCount ), fs::fs_error_t::OK, "list package overlay directory" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_FindFiles( "game", "*.cfg", fs::CYPHER_FILESYSTEM_FIND_RECURSIVE | fs::CYPHER_FILESYSTEM_FIND_FILES | fs::CYPHER_FILESYSTEM_FIND_SORT_BY_NAME, pPackageFindEntries, 8u, nPackageFindCount ), fs::fs_error_t::OK, "find files through package overlay" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_CopyFile( "game/package_only.cfg", "profiles/player1/package-copy.cfg" ), fs::fs_error_t::OK, "copy package file to write path" ) ) {
        return 1;
    }
    if ( !Check( fs::FS_FileExists( "profiles/player1/package-copy.cfg" ), "copied package file exists" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_DeleteFile( "profiles/player1/package-copy.cfg" ), fs::fs_error_t::OK, "delete copied package file" ) ) {
        return 1;
    }

    if ( !CheckError( fs::FS_UnmountPackage( szPackageArchivePathString.c_str() ), fs::fs_error_t::OK, "unmount package" ) ) {
        return 1;
    }
    if ( !Check( !fs::FS_PackageIsMounted( szPackageArchivePathString.c_str() ), "package is unmounted" ) ) {
        return 1;
    }
    pPackageReadBuffer[0] = '\0';
    if ( !CheckError( fs::FS_ReadEntireFile( "game/shared.cfg", pPackageReadBuffer, sizeof( pPackageReadBuffer ), nPackageBytesRead ), fs::fs_error_t::OK, "read loose fallback after package unmount" ) ) {
        return 1;
    }
    ...
    if ( !Check( fs::FS_Exists( "profiles/player1/config.cfg" ), "file exists through mount" ) ) {
        return 1;
    }
    if ( !Check( fs::FS_FileExists( "profiles/player1/config.cfg" ), "file exists as file" ) ) {
        return 1;
    }
    if ( !Check( fs::FS_DirectoryExists( "profiles/player1" ), "directory exists through mount" ) ) {
        return 1;
    }

    fs::resolve_trace_t trace{};
    if ( !CheckError( fs::FS_TraceResolve( "profiles/player1/config.cfg", trace ), fs::fs_error_t::OK, "trace resolve" ) ) {
        return 1;
    }
    ...
    fs::file_info_t info{};
    if ( !CheckError( fs::FS_GetFileInfo( "profiles/player1/config.cfg", info ), fs::fs_error_t::OK, "get file info" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_ListDirectory( "profiles/player1", entries, 8u, nEntryCount ), fs::fs_error_t::OK, "list directory" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_FindFiles( "profiles", "*.cfg", fs::CYPHER_FILESYSTEM_FIND_RECURSIVE | fs::CYPHER_FILESYSTEM_FIND_FILES | fs::CYPHER_FILESYSTEM_FIND_SORT_BY_NAME, entries, 8u, nEntryCount ), fs::fs_error_t::OK, "find files" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_CreateDirectory( "bulk" ), fs::fs_error_t::OK, "create bulk directory" ) ) {
        return 1;
    }
    for ( cypher::engine::common::u32 i = 0u; i < 270u; ++i ) {
        char szBulkFile[64]{};
        std::snprintf( szBulkFile, sizeof( szBulkFile ), "bulk/file%03u.tmp", i );
        if ( !CheckError( fs::FS_WriteEntireFile( szBulkFile, text, std::strlen( text ) ), fs::fs_error_t::OK, "write bulk file" ) ) {
            return 1;
        }
    }
    ...
    if ( !CheckError( fs::FS_FindFiles( "bulk", "*.tmp", fs::CYPHER_FILESYSTEM_FIND_FILES, pBulkEntries, 300u, nBulkEntryCount ), fs::fs_error_t::OK, "find bulk files" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_AppendEntireFile( "profiles/player1/config.cfg", szExtraText, std::strlen( szExtraText ) ), fs::fs_error_t::OK, "append entire file" ) ) {
        return 1;
    }

    char pReadBuffer[64]{};
    cypher::engine::common::u64 nBytesRead = 0u;
    if ( !CheckError( fs::FS_ReadEntireFile( "profiles/player1/config.cfg", pReadBuffer, sizeof( pReadBuffer ), nBytesRead ), fs::fs_error_t::OK, "read entire file" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_CopyFile( "profiles/player1/config.cfg", "profiles/player1/config-copy.cfg" ), fs::fs_error_t::OK, "copy file" ) ) {
        return 1;
    }
    if ( !Check( fs::FS_FileExists( "profiles/player1/config-copy.cfg" ), "copied file exists" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_DeleteFile( "profiles/player1/config-copy.cfg" ), fs::fs_error_t::OK, "delete copied file" ) ) {
        return 1;
    }

    if ( !CheckError( fs::FS_CreateDirectory( "profiles/player2/nested" ), fs::fs_error_t::OK, "create recursive directory" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_WriteEntireFile( "profiles/player2/nested/temp.cfg", text, std::strlen( text ) ), fs::fs_error_t::OK, "write entire file" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_RemoveDirectoryTree( "profiles/player2" ), fs::fs_error_t::OK, "remove directory tree" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_RemoveDirectoryTree( "bulk" ), fs::fs_error_t::OK, "remove bulk directory" ) ) {
        return 1;
    }

    fs::stats_t stats{};
    if ( !CheckError( fs::FS_GetStats( stats ), fs::fs_error_t::OK, "get stats" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_ReadAsync( "profiles/player1/config.cfg", pReadBuffer, sizeof( pReadBuffer ), request ), fs::fs_error_t::OK, "read async" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_WaitAsync( request, asyncResult ), fs::fs_error_t::OK, "wait read async" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_WriteAsync( "profiles/player1/async.cfg", szAsyncText, std::strlen( szAsyncText ), writeRequest ), fs::fs_error_t::OK, "write async" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_WaitAsync( writeRequest, asyncResult ), fs::fs_error_t::OK, "wait write async" ) ) {
        return 1;
    }
    ...
    std::memset( pReadBuffer, 0, sizeof( pReadBuffer ) );
    if ( !CheckError( fs::FS_ReadEntireFile( "profiles/player1/async.cfg", pReadBuffer, sizeof( pReadBuffer ), nBytesRead ), fs::fs_error_t::OK, "read async written file" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_WatchPath( "profiles/player1", fs::CYPHER_FILESYSTEM_WATCH_DIRECTORY, watch ), fs::fs_error_t::OK, "watch directory" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_PollChanges( events, 8u, nWatchEventCount ), fs::fs_error_t::OK, "poll unchanged watch" ) ) {
        return 1;
    }
    TraceStep( "watch write file" );
    if ( !CheckError( fs::FS_WriteEntireFile( "profiles/player1/watched.cfg", text, std::strlen( text ) ), fs::fs_error_t::OK, "write watched file" ) ) {
        return 1;
    }
    TraceStep( "watch poll created" );
    if ( !CheckError( fs::FS_PollChanges( events, 8u, nWatchEventCount ), fs::fs_error_t::OK, "poll created watch file" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_DeleteFile( "profiles/player1/watched.cfg" ), fs::fs_error_t::OK, "delete watched file" ) ) {
        return 1;
    }
    TraceStep( "watch poll deleted" );
    if ( !CheckError( fs::FS_PollChanges( events, 8u, nWatchEventCount ), fs::fs_error_t::OK, "poll deleted watch file" ) ) {
        return 1;
    }
    ...
    if ( !CheckError( fs::FS_UnwatchPath( watch ), fs::fs_error_t::OK, "unwatch directory" ) ) {
        return 1;
    }

    TraceStep( "cleanup" );

    if ( !CheckError( fs::FS_MountPackage( "", "pak0.pak", fs::CYPHER_FILESYSTEM_MOUNT_READ_ONLY | fs::CYPHER_FILESYSTEM_MOUNT_OPTIONAL, 0u ), fs::fs_error_t::OK, "optional missing package mount" ) ) {
        return 1;
    }

    if ( !CheckError( fs::FS_Rename( "profiles/player1/config.cfg", "profiles/player1/config2.cfg" ), fs::fs_error_t::OK, "rename file" ) ) {
        return 1;
    }
    if ( !Check( fs::FS_Exists( "profiles/player1/config2.cfg" ), "renamed file exists through mount" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_DeleteFile( "profiles/player1/config2.cfg" ), fs::fs_error_t::OK, "delete file" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_RemoveDirectory( "profiles/player1" ), fs::fs_error_t::OK, "remove player directory" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_RemoveDirectory( "profiles" ), fs::fs_error_t::OK, "remove profiles directory" ) ) {
        return 1;
    }
    if ( !CheckError( fs::FS_Unmount( pBaseMount ), fs::fs_error_t::OK, "unmount base overlay" ) ) {
        return 1;
    }
