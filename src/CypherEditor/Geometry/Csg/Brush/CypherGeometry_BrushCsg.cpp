    ...
    geometry_brush_piece_t *pOut ) noexcept
{
    ...
    vec3d_t unique[BRUSH_CSG_HULL_POINTS_MAX];
    usize cUnique = 0u;
    ...
    for ( usize i = 0u; i < points.nCount; ++i ) {
        ...
        if ( cUnique >= BRUSH_CSG_HULL_POINTS_MAX ) {
    ...
    scoped_piece_t hull{};
    if ( !hull.TryInit( AllocatorOf( pOut ) ) ) {
        return geometry_status_t::ALLOCATION_FAILED;
    }
        ...
        for ( usize j = i + 1u; j < cUnique; ++j ) {
            for ( usize k = j + 1u; k < cUnique; ++k ) {
                ...
                if ( maxDistance > tolerance ) {
                    plane = BrushCsg_FlipPlane( plane );
                }
                if ( maxDistance <= tolerance && minDistance >= -tolerance ) {
                    return geometry_status_t::DEGENERATE; // all points coplanar
                }
                bool_t bKnown = false;
                const usize cPlanes = BrushPiece_PlaneCount( &hull.piece );
                for ( usize p = 0u; p < cPlanes && !bKnown; ++p ) {
                    bKnown = SamePlane( hull.piece.planes.pData[p].plane, plane, tolerance );
                }
                if ( bKnown ) {
                    continue;
                }
                ...
                const geometry_status_t status =
                    BrushPiece_TryAppendPlane( &hull.piece, policy, facet );
                if ( status != geometry_status_t::OK ) {
                    return status;
                }
            }
    ...
    geometry_piece_extent_t extent = geometry_piece_extent_t::EMPTY;
    geometry_status_t status = BrushPiece_TryReduce( &hull.piece, policy, &extent, nullptr );
    if ( status != geometry_status_t::OK ) {
        return status;
    }
    if ( extent == geometry_piece_extent_t::EMPTY ) {
    ...
    return BrushPiece_TryCopy( pOut, &hull.piece );
}
